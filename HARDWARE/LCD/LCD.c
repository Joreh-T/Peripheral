...
void LCD_GPIO_Init(void) {
    ...
    RCC_APB2PeriphClockCmd(LCD_SCL_RCC | LCD_SDA_RCC | LCD_RES_RCC | LCD_DC_RCC | LCD_CS_RCC | LCD_BLK_RCC, ENABLE); //使能端口时钟
...
返回值：无
*************************************************/
void LCD_SetXY(u16 x, u16 y) { LCD_SetRegion(x, y, x, y); }
...
void LCD_Clear_XY(u16 Color, u16 X_Start, u16 Y_Start, u16 X_End, u16 Y_End) {
    unsigned int i, m;
    LCD_SetRegion(X_Start, X_Start, X_MAX_PIXEL - 1, Y_MAX_PIXEL - 1);
    LCD_WriteIndex(0x2C);
...
/*
void SWK_LCD_DrawNum(u16 x, u16 y, u16 fc, u16 bc, float num) {
    u8 TempIn[10];      // 整数部分+小数点
    u8 TempDe[3];       // 小数部分（3位小数）
    short i = 0, j = 0; // i会小于0，不能定义为无符号型
    u8 Res[11] = {0};   // 保存转换为字符型的数字
    u16 inte = 0;       // 整数部分
    u16 deci = 0;       // 小数部分
    TempIn[0] = '.';    //先把小数点放进去，后面反着取出来
    if (num > 1) {
        inte = (u16)num;
        deci = (u16)((num - inte) * 1000);
        for (i = 1; inte > 0; i++) {
            TempIn[i] = (inte % 10) + '0';
            inte /= 10;
        }
    ...
    LCD_DrawFont_GBK16(x, y, fc, bc, Res);
    // for(int te = 0; te <= 11; te++) {Res[i] = 0;}
}
*/
